...
- Images: `assets/` — `well.webp` is the wishing-well art from the VibeWell home page, positioned in the VibeWell card by the `--well-w` variable on `.vibe-link`; `mill.webp` is its amber watermill counterpart, positioned in the VibeMill card by `--mill-w` on `.vibemill-panel`. Both are transparent-background WebP cropped tight to the subject (`well.webp` 560×741, `mill.webp` 560×713); `.vibe-mill-art` sizes on width alone, so the differing aspect needs no extra handling.
